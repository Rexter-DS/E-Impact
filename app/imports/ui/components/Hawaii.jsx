...
class Hawaii extends React.Component {
  ...
  renderPage() {
    const nonCarArr = Trips.find({ county: 'Hawaii', mode: { $not: 'Gas Car' } }).fetch().map(function (element) {
    ...
    const carArr = Trips.find({ county: 'Hawaii', mode: 'Gas Car' }).fetch().map(function (element) {
    ...
    const bikeCount = _.size(Trips.find({ county: 'Hawaii', mode: 'Bike' }).fetch());
    const carpoolCount = _.size(Trips.find({ county: 'Hawaii', mode: 'Carpool' }).fetch());
    const evCount = _.size(Trips.find({ county: 'Hawaii', mode: 'Electric Vehicle' }).fetch());
    const carCount = _.size(Trips.find({ county: 'Hawaii', mode: 'Gas Car' }).fetch());
    const ptCount = _.size(Trips.find({ county: 'Hawaii', mode: 'Public Transportation' }).fetch());
    const teleworkCount = _.size(Trips.find({ county: 'Hawaii', mode: 'Telework' }).fetch());
    const walkCount = _.size(Trips.find({ county: 'Hawaii', mode: 'Walk' }).fetch());
