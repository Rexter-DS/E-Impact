...
class Kalawao extends React.Component {
  componentDidUpdate() {
  ...
  renderPage() {
    const nonCarArr = Trips.find({ county: 'Kalawao', mode: { $not: 'Gas Car' } }).fetch().map(function (element) {
    ...
    const carArr = Trips.find({ county: 'Kalawao', mode: 'Gas Car' }).fetch().map(function (element) {
    ...
    const bikeCount = _.size(Trips.find({ county: 'Kalawao', mode: 'Bike' }).fetch());
    const carpoolCount = _.size(Trips.find({ county: 'Kalawao', mode: 'Carpool' }).fetch());
    const evCount = _.size(Trips.find({ county: 'Kalawao', mode: 'Electric Vehicle' }).fetch());
    const carCount = _.size(Trips.find({ county: 'Kalawao', mode: 'Gas Car' }).fetch());
    const ptCount = _.size(Trips.find({ county: 'Kalawao', mode: 'Public Transportation' }).fetch());
    const teleworkCount = _.size(Trips.find({ county: 'Kalawao', mode: 'Telework' }).fetch());
    const walkCount = _.size(Trips.find({ county: 'Kalawao', mode: 'Walk' }).fetch());
