...
class Kauai extends React.Component {
  ...
  renderPage() {
    const nonCarArr = Trips.find({ county: 'Kauai', mode: { $not: 'Gas Car' } }).fetch().map(function (element) {
    ...
    const carArr = Trips.find({ county: 'Kauai', mode: 'Gas Car' }).fetch().map(function (element) {
    ...
    const bikeCount = _.size(Trips.find({ county: 'Kauai', mode: 'Bike' }).fetch());
    const carpoolCount = _.size(Trips.find({ county: 'Kauai', mode: 'Carpool' }).fetch());
    const evCount = _.size(Trips.find({ county: 'Kauai', mode: 'Electric Vehicle' }).fetch());
    const carCount = _.size(Trips.find({ county: 'Kauai', mode: 'Gas Car' }).fetch());
    const ptCount = _.size(Trips.find({ county: 'Kauai', mode: 'Public Transportation' }).fetch());
    const teleworkCount = _.size(Trips.find({ county: 'Kauai', mode: 'Telework' }).fetch());
    const walkCount = _.size(Trips.find({ county: 'Kauai', mode: 'Walk' }).fetch());
    ...
    return (
        <Grid centered>
          <Grid.Row>
            <Grid.Column as="h2" textAlign='center'>Kauai County</Grid.Column>
          </Grid.Row>
          <Grid.Row>
