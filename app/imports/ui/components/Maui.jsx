...
class Maui extends React.Component {
  ...
  renderPage() {
    const nonCarArr = Trips.find({ county: 'Maui', mode: { $not: 'Gas Car' } }).fetch().map(function (element) {
    ...
    const carArr = Trips.find({ county: 'Maui', mode: 'Gas Car' }).fetch().map(function (element) {
    ...
    const totalUsers = Meteor.users.find({ 'profile.county': 'Maui' }).count();

    const carDistances = _.map(Trips.find({ county: 'Maui', mode: 'Gas Car' }).fetch(), 'distance');
    const carMpgs = _.map(Trips.find({ county: 'Maui', mode: 'Gas Car' }).fetch(), 'mpg');
    ...
    const otherDistances = _.map(Trips.find({ county: 'Maui', mode: { $not: 'Gas Car' } }).fetch(), 'distance');
    const totalMilesSaved = _.sum(otherDistances).toFixed(2);
    const otherMpgs = _.map(Trips.find({ county: 'Maui', mode: { $not: 'Gas Car' } }).fetch(), 'mpg');
    ...
    const bikeCount = _.size(Trips.find({ county: 'Maui', mode: 'Bike' }).fetch());
    const carpoolCount = _.size(Trips.find({ county: 'Maui', mode: 'Carpool' }).fetch());
    const evCount = _.size(Trips.find({ county: 'Maui', mode: 'Electric Vehicle' }).fetch());
    const carCount = _.size(Trips.find({ county: 'Maui', mode: 'Gas Car' }).fetch());
    const ptCount = _.size(Trips.find({ county: 'Maui', mode: 'Public Transportation' }).fetch());
    const teleworkCount = _.size(Trips.find({ county: 'Maui', mode: 'Telework' }).fetch());
    const walkCount = _.size(Trips.find({ county: 'Maui', mode: 'Walk' }).fetch());
    ...
    return (
        <Grid centered>
          <Grid.Row>
            <Grid.Column as="h2" textAlign='center'>Maui County</Grid.Column>
          </Grid.Row>
          <Grid.Row>
